...
frappe.ui.form.on("Healthcare Package Order", {
  refresh: (frm) => {
    ...
    frm
      .get_field("consultations")
      .$wrapper.find(".grid-remove-all-rows")
      .hide();
  ...
  healthcare_package: (frm) => {
    if (frm.doc.healthcare_package) {
      frappe.db
        .get_doc("Healthcare Package", frm.doc.healthcare_package)
        .then((package_doc) => {
  ...
  create_sales_invoice: (frm) => {
    if (frm.is_dirty()) {
      frappe.msgprint(
        __("<b>Please save the document before creating Sales Invoice</b>")
      );
      return;
    }

    frm
      .call("create_sales_invoice", {
        self: frm.doc,
        freeze: true,
...
frappe.ui.form.on("Healthcare Package Order Consultation", {
  form_render: (frm, cdt, cdn) => {
    frm
      .get_field("consultations")
      .grid.wrapper.find(".grid-delete-row")
      .hide();
    frm.get_field("consultations").grid.wrapper.find(".grid-move-row").hide();
    frm
      .get_field("consultations")
      .grid.wrapper.find(".grid-duplicate-row")
      .hide();
    frm
      .get_field("consultations")
      .grid.wrapper.find(".grid-append-row")
      .hide();
    frm
      .get_field("consultations")
      .grid.wrapper.find(".grid-insert-row")
      .hide();
    frm
      .get_field("consultations")
      .grid.wrapper.find(".grid-insert-row-below")
      .hide();
  },
});
