...
frappe.ui.form.on("Patient Discount Request", {
  ...
  apply_discount_on: (frm) => {
    if (
      ["Grand Total", "Net Total", "Group of Items"].includes(
        frm.doc.apply_discount_on
      )
    ) {
  ...
  discount_percent: (frm) => {
    if (frm.doc.items.length > 0) {
      frm.doc.items.forEach((row) => {
        row.discount_amount =
          (row.actual_price * frm.doc.discount_percent) / 100;
...
var get_items = (frm, reference_dt, reset_options = null) => {
  frappe
    .call({
      method:
        "hms_tz.nhif.doctype.patient_discount_request.patient_discount_request.get_items",
      args: {
          ...
          if (!reset_options) {
            data.forEach((row) => {
              ...
              if (frm.doc.discount_percent) {
                discount_amount =
                  (row.amount * frm.doc.discount_percent) / 100;
...
var get_items_details = (frm, row) => {
  frappe
    .call({
      method:
        "hms_tz.nhif.doctype.patient_discount_request.patient_discount_request.get_item_details",
      args: {
        invoice_no: frm.doc.sales_invoice,
    ...
    .then((r) => {
      if (r.message) {
        ...
        if (data.length > 0) {
          ...
          if (frm.doc.discount_percent) {
            discount_amount =
              data[0].amount * (frm.doc.discount_percent / 100);
          } else if (frm.doc.discount_amount) {
            discount_amount = frm.doc.discount_amount;
          }
        ...
        if (data.slice(1).length > 0) {
          ...
          additonal_data.forEach((row) => {
            ...
            frappe.show_alert({
              message: __(
                "Additional items added to the list for same Item Code"
              ),
              indicator: "green",
              title: __("Success"),
            });
