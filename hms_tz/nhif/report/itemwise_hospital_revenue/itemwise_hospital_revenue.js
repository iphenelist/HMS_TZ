  ...
  filters: [
    {
      ...
      on_change: function () {
        ...
        frappe.call({
          method:
            "hms_tz.nhif.report.itemwise_hospital_revenue.itemwise_hospital_revenue.get_payment_modes",
          args: {
            company: company,
          },
          async: false,
    ...
    },
    {
      ...
      fieldtype: "Check",
      on_change: function () {
        let show_only_cancelled = frappe.query_report.get_filter_value(
          "show_only_cancelled_items"
        );
    ...
    },
    {
      ...
      fieldtype: "Check",
      description:
        "Show only items of ongoing admitted patient based on date range",
    },
    {
      ...
      fieldtype: "Check",
      description:
        "Show previous items of discharged patient based before start date",
      on_change: function () {
        let show_only_prev_items_for_discharged_ipds =
          frappe.query_report.get_filter_value(
            "show_only_prev_items_for_discharged_ipds"
          );
        let show_only_ongoing_ipds = frappe.query_report.get_filter_value(
          "show_only_ongoing_ipds"
        );
        if (
          show_only_prev_items_for_discharged_ipds == 1 &&
          show_only_ongoing_ipds == 1
        ) {
          frappe.msgprint(
            "Please select only one option either 'Show Ongoing IPDs' or 'Show Previous Items For Discharged IPDs'"
          );
        }
      },
    },
