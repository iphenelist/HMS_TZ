frappe.ui.form.on("Radiology Examination", {
  refresh: (frm) => {
    ...
    render_re_consumables_section(frm);
  },
  onload: (frm) => {
    $('[data-label="Not%20Serviced"]').parent().hide();
    if (frm.doc.patient) {
...
function render_re_consumables_section(frm) {
  if (!frm.fields_dict.re_consumables_html) return;

  if (frm.is_new() || !frm.doc.patient) {
    frm.fields_dict.re_consumables_html.$wrapper.html(
      '<div class="text-muted text-center p-4">' +
        __("Save the Radiology Examination first to add consumables.") +
        "</div>"
    );
    return;
  }

  const $wrapper = frm.fields_dict.re_consumables_html.$wrapper;
  $wrapper.empty();
  ...
  $('<button class="btn btn-primary btn-sm">')
    ...
    .on("click", () => {
      ...
      hms_tz.open_consumable_dialog({
        ...
        prescribed_by: "",
        source_doctype: "Radiology Examination",
