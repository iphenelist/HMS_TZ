...
def on_submit(doc, methd):
	...
	if doc.ot_schedule:
		frappe.db.set_value("OT Schedule", doc.ot_schedule, "status", "Completed")
...
def get_ot_schedule(doc):
	...
	ot_schedule = frappe.get_cached_value(
		"OT Schedule",
		{"patient": doc.patient, "company": doc.company, "procedure_template": doc.procedure_template},
		"name",
	)
	if not ot_schedule:
		return
	...
	for row in ot_schedule_doc.get("surgical_team"):
		doc.append("surgical_team", {"practitioner": row.practitioner, "role": row.role})
...
def get_preop_notes(doc):
	...
	pre_operative_note = frappe.get_cached_value(
		"Preoperative Assessment", {"clinical_procedure": doc.name}, "pre_operative_note"
	)

	if not pre_operative_note:
		pre_operative_note = frappe.get_cached_value(
			"Preoperative Assessment",
			{"patient": doc.patient, "ot_schedule": doc.ot_schedule, "service_name": doc.procedure_template},
			"pre_operative_note",
		)
...
def validate_swab_count(doc):
	...
	if swab_before and swab_before != swab_after:
		errors.append(_("Swab count mismatch: Before ({0}) ≠ After ({1})").format(swab_before, swab_after))
	...
	if errors and not doc.get("count_verified"):
		frappe.throw(
			_(
				"Count verification failed:<br>{0}<br><br>"
				"Please verify counts are correct and check 'Count Verified' to proceed."
...
def create_implant_registry(clinical_procedure: str, **kwargs) -> str:
	...
	frappe.db.set_value("Clinical Procedure", clinical_procedure, "implant", ir.name)
...
def create_surgical_specimen(clinical_procedure: str, **kwargs) -> str:
	...
	frappe.db.set_value("Clinical Procedure", clinical_procedure, "surgical_specimen", ss.name)
