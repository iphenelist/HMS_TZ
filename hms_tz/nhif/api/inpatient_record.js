frappe.ui.form.on("Inpatient Record", {
  ...
  refresh(frm) {
    ...
    $('*[data-fieldname="inpatient_occupancies"]')
    ...
    $('*[data-fieldname="inpatient_consultancy"]')
      .find(".grid-remove-rows")
      .hide();
    $('*[data-fieldname="inpatient_consultancy"]')
      .find(".grid-remove-all-rows")
    ...
    if (!frm.doc.insurance_subscription) {
      ...
      frm
        .add_custom_button(__("Make Deposit"), () => {
          make_deposit(frm);
        })
        .addClass("font-weight-bold");
    }
...
frappe.ui.form.on("Inpatient Occupancy", {
  form_render(frm, cdt, cdn) {
    frm.fields_dict.inpatient_occupancies.grid.wrapper
...
frappe.ui.form.on("Inpatient Consultancy", {
  form_render(frm, cdt, cdn) {
    frm.fields_dict.inpatient_consultancy.grid.wrapper
      .find(".grid-delete-row")
      .hide();
    frm.fields_dict.inpatient_consultancy.grid.wrapper
      .find(".grid-duplicate-row")
          ...
          if (r.message) {
            if (r.message.type != "Cash" && !values.reference_number) {
              frappe.msgprint({
                ...
                indicator: "red",
                message: __(
                  "Reference Number is required for non cash payments,<br>please enter reference number or change mode of payment to cash"
                ),
              });
              ...
              frappe.msgprint({
                ...
                indicator: "red",
                message: __(
                  "Reference Date is required for non cash payments,<br>please enter reference date or change mode of payment to cash"
                ),
              });
            } else {
              d.hide();
...
var validate_inpatient_balance_vs_inpatient_cost = (
  frm,
  row = null,
  fieldname = "",
  caller = ""
  ...
  if (!frm.doc.insurance_subscription) {
    ...
    frappe.call({
      method:
        "hms_tz.nhif.api.inpatient_record.validate_inpatient_balance_vs_inpatient_cost",
      args: {
        patient: frm.doc.patient,
