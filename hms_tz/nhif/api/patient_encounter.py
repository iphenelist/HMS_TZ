...
def on_submit_validation(doc, method):
	...
	if (
		not doc.healthcare_service_unit
		and not doc.healthcare_package_order
		and "Direct Cash" not in doc.practitioner
	):
...
def validate_totals(doc, method, show_alert=True):
	...
	if (
		"Jubilee" in doc.insurance_company
		and cint(frappe.get_cached_value("HMS TZ Setting", doc.company, "enable_jubilee_api")) == 1
	):
		return
...
def validate_medication_class(company, encounter, patient, drug_item, caller="Backend"):
	...
	validate_medication_class = frappe.get_cached_value(
		"HMS TZ Setting", company, "validate_medication_class"
	)
...
def set_practitioner_name(doc, method):
	...
	if practitioner_info:
		...
		if (
			...
			if validate_nhif_attendance == 1 and method not in ("before_insert", "validate", "before_save"):
...
def validate_medical_code(doc, method):
	...
	for from_table, fields in medical_code_mapping().items():
		...
		if not has_disease_code and len(doc.get(from_table)) > 0:
...
def validate_jubilee_pre_approval(doc):
	...
	if not frappe.get_cached_value("HMS TZ Setting", doc.company, "enable_jubilee_api"):
		return
