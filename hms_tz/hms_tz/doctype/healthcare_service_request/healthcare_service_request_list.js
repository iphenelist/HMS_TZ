frappe.listview_settings["Healthcare Service Request"] = {
  ...
  onload: (listview) => {
    ...
    listview.page
      .add_inner_button(__("Create Service Request"), () => {
    ...
    fields: [
      {
        ...
        options: [
          ...
          {
            label: __("Radiology Examination"),
            value: "Radiology Examination",
          },
    ...
    primary_action(values) {
      if (values) {
        if (values.source_doctype != "Patient Encounter") {
          ...
          frappe.call({
            method:
              "hms_tz.hms_tz.doctype.healthcare_service_request.healthcare_service_request.create_service_request",
            args: {
              data: values,
            },
            freeze: true,
            ...
            callback: (r) => {
              if (r.message) {
                ...
                frappe.show_alert(
                  {
                    message: __("{0} Service Request Created successfully", [
                      r.message,
                    ]),
                    indicator: "green",
                  },
