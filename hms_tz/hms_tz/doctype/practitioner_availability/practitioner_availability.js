  ...
  service_unit: function (frm) {
    if (frm.doc.service_unit) {
      frappe.db.get_value(
        ...
        { name: frm.doc.service_unit },
        "total_service_unit_capacity",
        (r) => {
          if (
            r.total_service_unit_capacity &&
            r.total_service_unit_capacity > 0
          ) {
    ...
    frappe.db.get_value(
      ...
      (r) => {
        if (r.total_service_unit_capacity) {
          if (
            r.total_service_unit_capacity < frm.doc.total_service_unit_capacity
          ) {
            frappe.show_alert({
...
var set_event_type_properties_to_event = function (frm) {
  if (frm.doc.availability_type) {
    frappe.db.get_value(
      "Practitioner Availability Type",
      { name: frm.doc.availability_type },
      [
        "availability_name",
        "present",
        "appointment_type",
    ...
    if (
      !frm.doc.total_service_unit_capacity &&
      !frm.doc.total_service_unit_capacity > 0
    ) {
