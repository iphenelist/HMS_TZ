...
from frappe.utils import flt, get_time, getdate, time_diff_in_seconds
...
class PatientAppointment(Document):
    ...
    def validate_overlaps(self):
        ...
        if self.service_unit:
            ...
            if allow_overlap:
                ...
                if service_unit_capacity and len(overlaps) >= int(service_unit_capacity):
                    frappe.throw(
                        _(f""" Not Allowed, Maximum capacity reached Service unit {self.service_unit}"""),
        ...
        if self.service_unit:
            service_unit_company = frappe.get_cached_value("Healthcare Service Unit", self.service_unit, "company")
    ...
    def update_fee_validity(self):
        ...
        if fee_validity:
            frappe.msgprint(_(f"{self.patient} has fee validity till {fee_validity.valid_till}"))
...
def invoice_appointment(appointment_doc):
    automate_invoicing = frappe.db.get_single_value("Healthcare Settings", "automate_appointment_invoicing")
    appointment_invoiced = frappe.get_cached_value("Patient Appointment", appointment_doc.name, "invoiced")
    ...
    if automate_invoicing and not appointment_invoiced and not fee_validity:
        ...
        frappe.msgprint(_(f"Sales Invoice {sales_invoice.name} created"), alert=True)
...
def cancel_appointment(appointment_id):
    ...
    if appointment.invoiced:
        ...
        if sales_invoice and cancel_sales_invoice(sales_invoice):
            ...
            msg = _(f"Appointment Cancelled. Please review and cancel the invoice {sales_invoice.name}")
        else:
...
def get_availability_data(date, practitioner):
    ...
    if practitioner_doc.practitioner_schedules or present_events:
        ...
        frappe.throw(
            _(f"{practitioner} does not have a Healthcare Practitioner Schedule. Add it in Healthcare Practitioner master"),
...
def get_available_slots(practitioner_doc, date):
    ...
    for schedule_entry in practitioner_doc.practitioner_schedules:
        if schedule_entry.schedule:
            ...
            frappe.throw(
                _(f"{frappe.bold(practitioner)} does not have a Healthcare Practitioner Schedule. Add it in Healthcare Practitioner"),
                title=_("Practitioner Schedule Not Found"),
            )
        ...
        if practitioner_schedule:
            ...
            if available_slots:
                ...
                if schedule_entry.service_unit:
                    ...
                    # fetch all appointments to practitioner without service
                    # unit
...
def update_status(appointment_id, status):
    ...
    procedure_prescription = frappe.get_cached_value("Patient Appointment", appointment_id, "procedure_prescription")
