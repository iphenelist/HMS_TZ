	...
	def validate_drug_row(self):
		if len(self.drug_items) > 0:
			...
			for row in self.drug_items:
				...
				if not row.reason:
					msg_print += (
						f"Reason for Return Field can not be Empty for drug name: {bold(row.drug_name)},\
                                Row: #{bold(row.idx)}:<br>"
					)
...
def update_drug_description_for_draft_delivery_note(self, delivery_note):
	try:
		...
		frappe.throw(
			str(
				f"Apply workflow error, for delivery note: {bold(delivery_note)}, check error log for more details"
			)
		)
		...
		for item in items:
			...
			if item.radiology_examination_template:
				...
				if item.radiology_examination:
					docstatus = frappe.get_cached_value(
						"Radiology Examination", item.radiology_examination, "docstatus"
					)
					if docstatus == 1:
			...
			if item.procedure:
				...
				if item.clinical_procedure:
					docstatus = frappe.get_cached_value(
						"Clinical Procedure", item.clinical_procedure, "docstatus"
					)
					if docstatus == 1:
...
def update_cancelled_hsr(
	ref_docname, ref_doctype, lrpmt_return_id, is_cancelled=1, qty_returned=0, remarks=""
):
	"""
	...
	is_prescribe = frappe.db.get_value(ref_doctype, ref_docname, "prescribe")

	if qty_returned > 0:
		if not is_prescribe:
			payment_records = (
				frappe.qb.from_(hsrp)
				.select(hsrp.name, hsrp.rate, hsrp.qty, hsrp.percent_covered)
			...
			for record in payment_records:
				...
				calculated_amount = round(
					record.get("rate", 0) * actual_qty * (record.get("percent_covered", 0) / 100), 2
				)
		...
		update_returned_or_cancelled_revenue_entry(
			ref_docname, "LRPMT Returns", lrpmt_return_id, is_cancelled=is_cancelled, remarks=remarks
		)
