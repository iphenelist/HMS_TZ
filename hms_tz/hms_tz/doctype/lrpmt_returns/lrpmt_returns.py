		...
		for item in items:
			if item.lab_test_code:
				...
				if not item.lab_test:
					name = get_refdoc("Lab Prescription", item.name, item.lab_test_code, item.parent)
					if name:
						lab_status = 'Draft'
					else:
						lab_status = ""
			...
			if item.procedure:
				...
				if not item.clinical_procedure:
					name = get_refdoc("Procedure Prescription", item.name, item.procedure, item.parent)
					if name:
	...
	for refd in ref_docs:
		if refd.get("table") == doctype:
			docname =  frappe.get_value(refd.get("ref_d"), {"ref_doctype": "Patient Encounter",
					"ref_docname": encounter, "hms_tz_ref_childname": childname, refd.get("field"): template}, ["name"]
				)
			return docname or ""
