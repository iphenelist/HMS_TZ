...
frappe.ui.form.on("LRPMT Returns", {
  ...
  onload: function (frm) {
    ...
    frm.validate_quantity_to_return = function (frm, row) {
      frm.doc.drug_items.forEach((data) => {
        if (
          data.quantity_to_return >
          data.quantity_prescribed - data.qty_returned
        ) {
    ...
    frm.validate_no_of_sessions_to_cancel = function (frm, row) {
      frm.doc.therapy_items.forEach((data) => {
        if (
          data.sessions_to_cancel >
          data.sessions_prescribed - data.sessions_cancelled
        ) {
  ...
  get_lrp_items: function (frm) {
    ...
    if (frm.doc.patient && frm.doc.appointment && frm.doc.company) {
      ...
      frappe.call({
        method:
          "hms_tz.hms_tz.doctype.lrpmt_returns.lrpmt_returns.get_lrp_item_list",
        args: {
          patient: frm.doc.patient,
        ...
        callback: (data) => {
          if (data.message.length > 0) {
            ...
            for (let i = 0; i < data.message.length; i++) {
              $results.append(
                make_lrp_list_row(columns, true, data.message[i])
              );
            }
          } else {
  ...
  get_therapy_items: (frm) => {
    ...
    if ((frm.doc.patient, frm.doc.appointment, frm.doc.company)) {
      ...
      frappe.call({
        method:
          "hms_tz.hms_tz.doctype.lrpmt_returns.lrpmt_returns.get_therapies",
        args: {
          patient: frm.doc.patient,
          appointment: frm.doc.appointment,
          company: frm.doc.company,
  ...
  get_drug_items: function (frm) {
    ...
    if ((frm.doc.patient, frm.doc.appointment, frm.doc.company)) {
      ...
      frappe.call({
        method:
          "hms_tz.hms_tz.doctype.lrpmt_returns.lrpmt_returns.get_drug_item_list",
        args: {
          patient: frm.doc.patient,
          appointment: frm.doc.appointment,
          company: frm.doc.company,
    ...
    .map(function () {
      ...
      if ($(this).find(".list-row-check:checkbox:checked").length > 0) {
        ...
        if ($(this).attr("data-reference_doctype") != "undefined") {
          checked_items["reference_doctype"] = $(this).attr(
        ...
        if ($(this).attr("data-reference_docname") != "undefined") {
          checked_items["reference_docname"] = $(this).attr(
...
var add_to_lrp_line = function (frm, checked_items, item) {
  if (item) {
    frappe.call({
      method:
        "hms_tz.hms_tz.doctype.lrpmt_returns.lrpmt_returns.set_checked_lrp_items",
      args: {
        doc: frm.doc,
...
var make_drug_list_row = function (columns, item, result = {}) {
  ...
  columns.forEach(function (column) {
    let contentWidth =
      column === "qty_prescribed" ||
      column === "qty_returned" ||
      column === "status"
        ? 100
        : 160; // Set width based on column name
...
var add_to_drug_line = function (frm, checked_items, item) {
  if (item) {
    frappe.call({
      method:
        "hms_tz.hms_tz.doctype.lrpmt_returns.lrpmt_returns.set_checked_drug_items",
      args: {
        doc: frm.doc,
        checked_items: checked_items,
      },
  ...
  return $results
    ...
    .map(function () {
      ...
      if ($(this).find(".list-row-check:checkbox:checked").length > 0) {
        ...
        checked_items["therapy_session"] = $(this).attr(
          "data-therapy_session"
        );
        checked_items["sessions_cancelled"] = $(this).attr(
          "data-sessions_cancelled"
        );
        checked_items["plan_child_table_id"] = $(this).attr(
          "data-plan_child_table_id"
        );
        checked_items["encounter_child_table_id"] = $(this).attr(
          "data-encounter_child_table_id"
        );
        return checked_items;
      }
    })
...
var add_to_therapy_line = function (frm, checked_items, item) {
  if (item) {
    frappe.call({
      method:
        "hms_tz.hms_tz.doctype.lrpmt_returns.lrpmt_returns.set_checked_therapy_items",
      args: {
        doc: frm.doc,
        checked_items: checked_items,
      },
