...
frappe.ui.form.on("Patient", {
  refresh: function (frm) {
    frm.set_query("patient", "patient_relation", function () {
      return {
        filters: [["Patient", "name", "!=", frm.doc.name]],
      };
    });
    ...
    if (
      frappe.defaults.get_default("collect_registration_fee") &&
      frm.doc.status == "Disabled"
    ) {
    ...
    if (
      ...
      frappe.user.has_role("Healthcare Practitioner")
    ) {
    ...
    if (!frm.is_new()) {
      if (
        frappe.user.has_role("Healthcare Nursing") ||
        frappe.user.has_role("Healthcare Practitioner")
      ) {
    ...
    frappe.call({
      ...
      callback: function (r) {
        frm.dashboard.add_indicator(
          __("Total Billing: {0}", [format_currency(r.message.total_billing)]),
          "blue"
        );
        frm.dashboard.add_indicator(
          __("Patient Balance: {0}", [
            format_currency(r.message.party_balance),
          ]),
          "orange"
        );
      },
...
frappe.ui.form.on("Patient Relation", {
  patient_relation_add: function (frm) {
    frm.fields_dict["patient_relation"].grid.get_field("patient").get_query =
      function (doc) {
        let patient_list = [];
