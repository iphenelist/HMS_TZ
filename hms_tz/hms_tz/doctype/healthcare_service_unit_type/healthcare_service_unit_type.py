...
class HealthcareServiceUnitType(Document):
	def validate(self):
		if self.allow_appointments and self.inpatient_occupancy:
			frappe.msgprint(
				_(
					f"Healthcare Service Unit Type cannot have both {frappe.bold('Allow Appointments')} and {frappe.bold('Inpatient Occupancy')}"
				),
				raise_exception=1,
			...
			frappe.msgprint(
				_(
					f"Healthcare Service Unit Type must allow atleast one among {frappe.bold('Allow Appointments')} and {frappe.bold('Inpatient Occupancy')}"
				),
				raise_exception=1,
				title=_("Validation Error"),
				indicator="red",
