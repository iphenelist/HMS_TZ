...
frappe.ui.form.on("Lab Test", {
  ...
  refresh: function (frm) {
    ...
    if (frm.doc.workflow_state === "Sample Analyzed") {
      if (
        frm.doc.descriptive_test_items &&
        frm.doc.descriptive_test_items.length > 0
      ) {
        frm.doc.descriptive_test_items.forEach(function (row) {
          if (!row.result_component_option) {
            frappe.throw(
              `Error: Result value is missing for <b>${row.lab_test_particulars}</b>`
            );
          }
        });
      } else if (
        frm.doc.normal_test_items &&
        frm.doc.normal_test_items.length > 0
      ) {
        frm.doc.normal_test_items.forEach(function (row) {
          if (!row.result_value) {
            frappe.throw(
              `Error: Result value is missing for <b>${row.lab_test_name}</b>`
            );
          }
        });
      }
...
var get_lab_test_prescribed = function (frm) {
  if (frm.doc.patient) {
    frappe.call({
      method:
        "hms_tz.hms_tz.doctype.lab_test.lab_test.get_lab_test_prescribed",
      args: { patient: frm.doc.patient },
      callback: function (r) {
    ...
    var row = $(
      repl(
			...
			</div><hr>',
        {
          ...
          invoiced: y[3],
          practitioner: y[4],
          practitioner_name: y[5],
          date: y[6],
          source: y[7],
          referring_practitioner: y[8],
        }
      )
    ).appendTo(html_field);
