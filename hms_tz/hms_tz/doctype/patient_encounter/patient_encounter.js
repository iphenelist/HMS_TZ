...
frappe.ui.form.on("Patient Encounter", {
  ...
  refresh: function (frm) {
    ...
    if (frm.doc.patient) {
      ...
      frm.fields_dict.patient_vitals.html(
        "<div align='center'>Please select Patient.</div>"
      );
    }
    if (!frm.doc.__islocal) {
      if (frm.doc.docstatus === 1) {
        if (
          ...
          frm.doc.inpatient_status != "Discharge Scheduled" &&
...
var schedule_inpatient = function (frm) {
  ...
  var dialog = new frappe.ui.Dialog({
    ...
    primary_action: function () {
      ...
      frappe.call({
        method:
          "hms_tz.hms_tz.doctype.inpatient_record.inpatient_record.schedule_inpatient",
        args: {
          args: args,
        },
        callback: function (data) {
...
var schedule_discharge = function (frm) {
  var dialog = new frappe.ui.Dialog({
    ...
    primary_action: function () {
      ...
      frappe.call({
        method:
          "hms_tz.hms_tz.doctype.inpatient_record.inpatient_record.schedule_discharge",
        args: { args },
        callback: function (data) {
          if (!data.exc) {
            frm.reload_doc();
...
var show_patient_vital_charts = function (
  patient,
  frm,
  btn_show_id,
  pts,
    ...
    callback: function (r) {
      if (r.message) {
        ...
        var show_chart_btns_html =
          "<div style='padding-top:5px;'><a class='btn btn-default btn-xs btn-show-chart' \
        ...
        frm.fields_dict.patient_vitals.$wrapper
          .find(".show_chart_btns")
          .html(show_chart_btns_html);
        // me.page.main.find(".show_chart_btns").html(show_chart_btns_html);
        //handler for buttons
        frm.fields_dict.patient_vitals.$wrapper
          .find(".btn-show-chart")
          .on("click", function () {
            var btn_show_id = $(this).attr("data-show-chart-id"),
...
var refer_practitioner = function (frm) {
  var dialog = new frappe.ui.Dialog({
    ...
    primary_action: function () {
      ...
      frappe.call({
        method:
          "hms_tz.hms_tz.doctype.patient_encounter.patient_encounter.create_patient_referral",
        args: { args },
        callback: function (data) {
          if (!data.exc) {
            frm.reload_doc();
