...
var practitioner_login_out_to_from_nhif = (listview) => {
    ...
    frappe.call({
        ...
        callback: (r) => {
			...
			if (listview.page.sidebar.find('.nhif-buttons').length > 0) {
				return;
			}

            let $filter_section = listview.page.sidebar.find(".sidebar-section.filter-section");
            if ($filter_section.length > 0) {
                let $container = $(`
                    <div class="sidebar-section nhif-buttons" style="margin: 10px 0;">
                ...
                `);
                $filter_section.before($container);
