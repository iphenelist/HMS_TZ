...
class NurseRecord(Document):
	def before_insert(self):
		if self.appointment:
			insurance = frappe.get_cached_value("Patient Appointment", self.appointment, "insurance_company")
			if insurance:
	...
	def set_missing_values(self):
		...
		if self.appointment and self.payment_type == "Insurance" and not self.insurance_subscription:
	...
	def validate_no_duplicate(self):
		...
		if existing:
			frappe.throw(
				_("A Nurse Record already exists for patient {0} with nurse {1} on {2}: {3}").format(
					self.patient, self.nurse, self.posting_date, existing
				)
			)
	...
	def validate_room_and_ward_fields(self):
		...
		if not self.room and not self.ward:
			frappe.throw(_("Please set at least one of Room or Ward."))

	def set_status_on_save(self):
		"""Auto-update status based on content."""
		if self.status == "Open" and (self.care_plans or self.nursing_notes or self.observations):
...
@frappe.whitelist()
def update_medication_status(imo_entry_name, status, notes=None, nurse_record=None):
...
def recalculate_remaining_schedules(imo_name, drug, reference_date, scheduled_time, actual_time):
...
def get_upcoming_medications(nurse, within_minutes=60):
	...
	for nr in nurse_records:
		...
		shift_start_dt = datetime.strptime(f"{posting} {shift_start_str}", "%Y-%m-%d %H:%M:%S")

		if shift_end <= shift_start:
			# Overnight shift — shift_end is on the next day
			next_day = add_days(posting, 1)
			shift_end_dt = datetime.strptime(f"{next_day} {shift_end_str}", "%Y-%m-%d %H:%M:%S")
		else:
			shift_end_dt = datetime.strptime(f"{posting} {shift_end_str}", "%Y-%m-%d %H:%M:%S")
...
def create_imo_from_delivery_note(doc, method):
	...
	if not frappe.db.get_value("HMS TZ Setting", doc.company, "auto_create_imo_from_delivery_note"):
		return
	...
	appointment = frappe.db.get_value("Patient Encounter", encounter_name, "appointment")
...
def create_medication_schedule(
	...
	if total_doses <= 0:
		frappe.throw(
			_("Cannot create a schedule: total quantity ({0}) is less than one dose ({1}).").format(
				total_administrable_qty, dose_per_administration
			)
		)
	...
	if imo_name:
		...
		imo.patient_name = frappe.db.get_value("Patient", dm.patient, "patient_name") or ""
	...
	frappe.delete_doc("Dispensed Medication", dispensed_medication, ignore_permissions=True)
