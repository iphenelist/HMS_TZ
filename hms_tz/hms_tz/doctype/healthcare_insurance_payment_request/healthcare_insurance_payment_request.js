...
var get_insurance_claim = function (frm) {
  ...
  if (frm.doc.insurance_company) {
    ...
    frappe.call({
      method:
        "hms_tz.hms_tz.doctype.healthcare_insurance_payment_request.healthcare_insurance_payment_request.get_claim_item",
      args: args,
      callback: function (data) {
        if (data.message) {
          data.message.forEach(function (claim) {
            var child_item = frappe.model.add_child(
              frm.doc,
            ...
            frappe.model.set_value(
              ...
              claim.healthcare_service_type
            );
            frappe.model.set_value(
              child_item.doctype,
              child_item.name,
              "service_template",
              claim.service_template
            );
            frappe.model.set_value(
              child_item.doctype,
              child_item.name,
