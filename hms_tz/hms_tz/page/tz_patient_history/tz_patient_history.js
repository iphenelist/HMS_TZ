...
class PatientHistory {
  ...
  make_patient_profile() {
    ...
    this.main_section
      .empty()
      .append(frappe.render_template("tz_patient_history"));
    this.setup_filters();
  ...
  setup_documents(
    ...
    if (selected_practitioner) filters["practitioner"] = selected_practitioner;

    let me = this;
    frappe.call({
      method:
        "hms_tz.hms_tz.page.tz_patient_history.tz_patient_history.get_feed",
      args: filters,
      callback: function (r) {
        let data = r.message;
        if (data.length) {
  ...
  show_patient_info() {
    this.get_patient_info().then(() => {
      $(".patient-info")
        .empty()
        .append(
          frappe.render_template("tz_patient_history_sidebar", {
