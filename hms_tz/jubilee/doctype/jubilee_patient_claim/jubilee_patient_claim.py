...
class JubileePatientClaim(Document):
	...
	def after_insert(self):
		...
		if len(items) > 0:
			frappe.set_value(self.doctype, self.name, "original_jubilee_patient_claim_item", items)
			self.reload()
	...
	def on_trash(self):
		...
		jubilee_patient_claim = frappe.db.get_value(
			...
			"jubilee_patient_claim",
		)
		if jubilee_patient_claim == self.name:
			frappe.qb.update(pa).set(pa.jubilee_patient_claim, "").where(
				pa.name == self.patient_appointment
			).run()
	...
	def get_patient_encounters(self):
		...
		patient_encounters = (
			...
			.where((pe.docstatus == 1) & (pe.appointment.isin(appointments)))
			.orderby(pe.creation)
		).run(as_dict=True)
	...
	def set_patient_claim_disease(self, encounter_list):
		...
		diagnosis_list = (
			...
			.where((ct.parenttype == "Patient Encounter") & (ct.parent.isin(encounter_ids)))
			...
			for occupancy in occupancy_list:
				...
				for d in encounter_list:
					...
					service_request_doc = frappe.get_doc(
						"Healthcare Service Request", d.get("service_request")
					)
					for row in service_request_doc.get("payments"):
						self.add_LRPMT_claim_item(row, d)
	...
	def set_clinical_notes(self, encounter):
		if not self.clinical_notes:
			...
			years = f"Age: <b>{(date_diff(nowdate(), self.date_of_birth)) // 365} years</b>,"
		...
		self.clinical_notes += (
			f"<br>PractitionerName: <i>{encounter_doc.practitioner_name},</i> Speciality: <i>{department},\
		...
		if len(encounter_doc.get("radiology_procedure_prescription")) > 0:
			...
			for row in encounter_doc.get("radiology_procedure_prescription"):
				...
				self.clinical_notes += (
					f"Radiology Name: <b>{row.radiology_procedure_name}</b> {serv_info} <br>"
				)
		...
		for app_name in appointment_list:
			if app_name["hms_tz_claim_appointment_list"]:
				...
				for d in app_numbers:
					frappe.qb.update(pa).set(pa.jubilee_patient_claim, self.name).where(pa.name == d).run()
			else:
	...
	def validate_multiple_appointments_per_authorization_no(self, caller=None):
		...
		if len(appointment_documents) > 1:
			self.validate_hold_card_status(appointment_documents, claim_details, merged_appointments, caller)
		elif caller:
	...
	def validate_hold_card_status(
		...
		msg += (
			f"<br> with same authorization no: <b>{self.authorization_no}</b><br><br> Please consider <strong>merging of claims</strong>\
	...
	def get_radiology_results(self, row):
		...
		radiology_report = frappe.get_cached_value(
			"Radiology Examination", row.radiology_examination, "radiology_report_details"
		)
		result = ""
		if radiology_report:
			result += "<div align='center'><i><u><b>Radiology Report</b></u></i></div>"
			result += "<table border='1' cellpadding='5' cellspacing='0' width='90%' align='center'>"
	...
	def get_procedure_notes(self, row):
		...
		result = ""
		if notes:
			result += "<div align='center'><i><u><b>Procedure Notes</b></u></i></div>"
			result += "<table border='1' cellpadding='5' cellspacing='0' width='90%' align='center'>"
	...
	def send_jubilee_claim(self):
		json_data, json_data_wo_files = self.get_payload()
		setting_doc = frappe.get_cached_doc("HMS TZ Setting", self.company)
		...
		url = str(setting_doc.jubilee_url) + "/jubileeapi/SendClaim"
		...
		try:
			...
			if r.status_code != 200:
				...
				json_start = raw.find('{"')
				if json_start == -1:
					frappe.throw(f"Jubilee returned an unreadable response:<br><br>{raw}")

				data = json.loads(raw[json_start:])
				if data.get("status") == "ERROR" or data.get("Status") == "ERROR":
					frappe.throw(data.get("description") or data.get("Description") or str(data))
			...
			add_jubilee_log(
				...
				status_code=(r.status_code if str(r) else "NO STATUS CODE r. Timeout???"),
				ref_doctype=self.doctype,
				ref_docname=self.name,
				company=self.company,
			)
	...
	def get_payload(self):
		...
		if self.patient_type_code == "IN":
			entities.DateAdmitted = str(self.date_admitted) + " " + str(self.admitted_time)
			entities.DateDischarged = str(self.date_discharge) + " " + str(self.discharge_time)
		...
		entities.LateAuthorizationReason = get_appointment_remarks(self.patient_appointment)
		entities.EmergencyAuthorizationReason = get_appointment_remarks(self.patient_appointment)
		...
		for disease in self.jubilee_patient_claim_disease:
			...
			FolioDisease.Remarks = disease.description or ""
	...
	def reconcile_repeated_items(self):
		...
		self.jubilee_patient_claim_item = reconcile_items(self.jubilee_patient_claim_item)
		self.original_jubilee_patient_claim_item = reconcile_items(self.original_jubilee_patient_claim_item)
...
def get_claim_pdf_file(doc):
	...
	html = frappe.get_print(doc.doctype, doc.name, print_format, doc=None, no_letterhead=1)
