...
frappe.ui.form.on("Jubilee Approval Request", {
  ...
  send_to_jubilee: (frm) => {
    if (!frm.is_new()) {
      ``;
      frm.add_custom_button("Send to Jubilee", () => {
        frappe.call({
          method: "send_to_jubilee",
          doc: frm.doc,
          freeze: true,
          freeze_message: __("Sending to Jubilee..."),
          callback: function (r) {
            if (r.message) {
              let data = r.message;
          ...
          onerror: function () {
            ...
            frappe.msgprint({
              ...
              indicator: "red",
              message: __(
                "An unexpected error occurred while sending the pre-authorization to Jubilee."
              ),
            });
          },
        });
  ...
  get_preauth_status: (frm) => {
    if (frm.doc.preauth_status && frm.doc.preauth_status != "ERROR") {
