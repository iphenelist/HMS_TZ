...
def get_preauth_entities(source):
	...
	entities.Age = str(date_diff(nowdate(), source.date_of_birth) // 365) if source.date_of_birth else "0"
	...
	return frappe._dict(
		{
			...
			or "",
			"provider_id": frappe.get_cached_value("HMS TZ Setting", hsr_doc.company, "jubilee_provider_id")
			or "",
...
def get_service_request_preauth_status(service_request_name):
	...
	if not hsr_doc.submission_id:
		frappe.throw(
			_("This Service Request has no Pre-Auth Submission ID. Please send the pre-authorization first.")
		)
	...
	try:
		...
		if isinstance(description, dict):
			approval_status = (
				description.get("PreauthorizationStatus") or description.get("preathorizationStatus") or ""
			)
	...
	if approval_status != "ERROR":
		desc = f"Requested Amount: <b>{requested_amount}</b> | Approved Amount: <b>{approved_amount}</b>"
	else:
